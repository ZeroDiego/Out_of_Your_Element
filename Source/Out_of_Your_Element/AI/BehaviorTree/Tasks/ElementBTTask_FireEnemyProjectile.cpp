...
EBTNodeResult::Type UElementBTTask_FireEnemyProjectile::ExecuteTask(UBehaviorTreeComponent& OwnerComp, uint8*)
{
	...
	if (!AI->GetWorldTimerManager().IsTimerActive(ResetFireHandle))
	{
		...
		FTimerDelegate ResetDel = FTimerDelegate::CreateWeakLambda(
			AI /*owner tracked "weakly" by delegate, Motvärkar memory leaks*/, [WeakBB]()
			{
	...
	if (Proj)
	{
		...
		//valid "UProjectileMovementComponent"?, "Movement component" som finns i "Projectile component" i blueprinten
		{
